...
llm = ChatOllama(model="llama3-chinese", device="cuda")
...
messages = [
    ...
    (
        ...
        """
        客戶詳細資訊: {target_info}
        基於FAISS相似度檢索，與此客戶類似的客群購買了這些種類的產品，FAISS檢索資料: {result_summalize}
        按照FAISS檢索資料以及客戶詳細資訊，適合推薦他什麼產品？
        """
    )
    ...
    # HumanMessage(content="{result_summalize}"),
    # HumanMessage(
    #     content="按照FAISS檢索資料以及客戶詳細資訊，適合推薦他什麼產品？"
    # ),
]
...
prompt_info = {"target_info": info.to_markdown(),
                    "result_summalize":dumps_sum.to_markdown(),
                    # "result": dumps.to_markdown(),
                    "reference": Reference}
try:print(llm.model_name)
except:print(llm.model)
# print('Target Info: ', prompt_info['target_info'])
# print('Search Result: ', prompt_info['result_summalize'])
print(chain.invoke(prompt_info))